...
const AuditEquipment = () => {
  ...
  // Populate form if audit data exists
  useEffect(() => {
    if (audit && id && isEditing) {
      console.log("Populating form with audit data:", audit);
      
      // Handle existing images differently from new uploads
      if (audit.image) {
        if (Array.isArray(audit.image)) {
          setExistingImages(audit.image);
        } else if (typeof audit.image === 'string') {
          setExistingImages([audit.image]);
        }
      }
